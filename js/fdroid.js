...
Element.prototype.getPath = function (name) {
    var sp = name.split("/");
    var x = this.getElementsByTagName(sp[0])[0];
    if (!x) return {textContent: null};
    if (sp.length == 1)return x;
    return x.getPath(sp.slice(1).join("/"));
};

function getRepoInfo(baseUrl, callback) {
    getXml(baseUrl + "/index.xml", function (xml) {
        var repoinfo = {};
        ...
        repoinfo.pubkey_sha256 = _shalib.sha256(repoinfo.pubkey).match(/.{1,2}/g).join(" ").toUpperCase();
        repoinfo.url = _repo.getAttribute("url");
        repoinfo.url_with_key = repoinfo.url + "?fingerprint=" + _shalib.sha256(repoinfo.pubkey);
        ...
        repoinfo.description = _repo.getPath("description").textContent;
        repoinfo.packages = {};
        ...
        for(var i = 0; i < pkgs.length; i++) {
            ...
            app.name = _pkg.getPath("name").textContent;
            app.summary = _pkg.getPath("summary").textContent;
            app.description_html = _pkg.getPath("desc").textContent;
            app.description_text = app.description_html.replace(/(<([^>]+)>)/ig,"");
            app.icon_src = baseUrl + "/icons/" + _pkg.getPath("icon").textContent;
            app.lastupdate = _pkg.getPath("lastupdate").textContent;
            app.latestvercode = Array.prototype.map.call(_pkg.getElementsByTagName("package"), function (pkg) {
                return parseInt(pkg.getPath("versioncode").textContent);
            }).sort().reverse()[0];
            app.latestvername = Array.prototype.filter.call(_pkg.getElementsByTagName("package"), function (pkg) {
                return parseInt(pkg.getPath("versioncode").textContent) == app.latestvercode;
            })[0].getPath("version").textContent;
            app.marketvercode = parseInt(_pkg.getPath("marketvercode").textContent);
            var marketver = Array.prototype.filter.call(_pkg.getElementsByTagName("package"), function (pkg) {
                return parseInt(pkg.getPath("versioncode").textContent) == app.marketvercode;
            })[0];
            app.marketvername = marketver ? marketver.getPath("version").textContent : "";
            app.versions = Array.prototype.reduce.call(_pkg.getElementsByTagName("package"), function (total, pkg) {
                total[parseInt(pkg.getPath("versioncode").textContent)] = pkg.getPath("version").textContent;
                return total;
            }, {});
...
function createEntry(name, value) {
    ...
    span_attrib.classList.add("attrib");
...
function appendRepoInfo(baseUrl, container, title) {
    getRepoInfo(baseUrl, function (repoinfo) {
        ...
        div_content.classList.add("repo-desc");
